...
- `.forgejo/workflows/` — CI на Forgejo Actions: `commitlint`, `gitleaks`,
  ...
  `actionlint` (валидация самих workflow).
...
но поддерживаются в актуальном состоянии как расшифровываемый реестр значений:
SealedSecret необратим, а `secrets.enc.env` позволяет достать исходные
значения. Приватный age-ключ существует только на сервере, в Git лежит лишь
публичный.

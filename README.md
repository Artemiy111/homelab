...
1. корневой `.env` — копия `.env.example` (`DOMAIN`, `TZ`, `SERVER_IP`);
2. трекаемый plaintext `apps/<сервис>/config.env`;
3. секреты `apps/<сервис>/secrets.enc.env` — расшифровываются через
   `sops exec-env` только в памяти процесса (plaintext-файлы не создаются;
   у немигрированных на SOPS сервисов локальный `.env` генерирует `init.sh`);

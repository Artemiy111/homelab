...
Инициализация создаёт каталоги и генерирует секреты в зашифрованный
`secrets.enc.env`. Пароли генерируются только при первом создании и не перезаписываются:
...
`INIT_SEAFILE_ADMIN_PASSWORD`. Она создаётся только при первом
...
`init.sh` генерирует общий JWT-секрет для Seafile и ONLYOFFICE. Существующие
секреты автоматически дополняются настройками редактора без замены. Настройки редактора для Seahub живут в отслеживаемом файле
...
`seahub_onlyoffice.py` или переменных `ONLYOFFICE_*` выполните
...
для повседневной работы. Секреты уже зашифрованы в репозитории — не расшифровывайте их в файлы на диске.
...
Сначала создайте дамп базы и Restic snapshot. Версию образа меняйте в `config.env`; существующие секреты `init.sh`

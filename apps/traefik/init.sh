...
# service_run (это делает bootstrap-platform.sh; вручную см.
# scripts/compose-secrets.sh). Plaintext-файл .env не создаётся.
if [[ -z "${RFC2136_TSIG_SECRET:-}" ]]; then
  ...
  echo "или: bash scripts/bootstrap-platform.sh traefik" >&2
  exit 1
fi
...
render_template \
  "$repo_root/apps/traefik/traefik.tpl.yaml" \
  "$repo_root/apps/traefik/traefik.yaml"

compose_config "$repo_root/apps/traefik"

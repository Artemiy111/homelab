...
> Traefik, VIP `.253` больше не используется (см. `platform/k0s/README.md`). Правило
...
общий вход (`.253`), и до него через Tailscale не достучаться.

## Корень проблемы

MetalLB в L2/ARP-режиме выдаёт LoadBalancer-сервису виртуальный LAN-адрес (`.253`).
Этот VIP обслуживается только в ARP-плоскости физической LAN (`enp2s0`):

- с **LAN** пакеты на `.253` приходят на сетевой интерфейс, срабатывает ARP сервера
  и DNAT/kube-proxy — трафик попадает в Traefik pod;
- с **Tailscale** пакеты уже «внутри» хоста (приходят на `tailscale0`) — а Linux-ядро
  не имеет `.253` как локального адреса и не слушает его там, поэтому шлёт
...
Гипотеза: «сделать `.253` локальным адресом на tailscale-интерфейсе» — тогда ядро
...
Результат:
- `.253` появился на `tailscale0`;
- `ip route get <node1-ip>` → `local ... dev lo table local` — ядро перестало
  рассматривать `.253` как вход LoadBalancer и начало слать пакеты в loopback;
- на `.253:443` **нет слушателя** (`ss` показывает слушателей только на
...
как процесс, слушающий `.253:443`. Делая `.253` локальным, мы ломаем механизм входа,
а не чиним его.
...
Эксперимент с `.253` на `tailscale0` был неверной гипотезой. Настоящая причина —

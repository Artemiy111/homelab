...
| `zitadel` | `zitadel` | `zitadel` | `zitadel` | 20 МБ |
| `immich` | `immich` | `immich` | `immich` | 287 МБ |
| `dawarich` | `dawarich` | `dawarich_production` | `dawarich` | 93 МБ |
...
1. **Версия и расширения** — не обойти: один `imageName` = один мажор и один
   набор расширений.
2. **Blast radius** — `zitadel` отдельно: падение IdP роняет всё, что логинится.
3. **Профиль нагрузки** — `immich` тяжелее всех остальных вместе.
...
имеет смысл включать выборочно, начиная с `zitadel`.
...
«На вырост» не резервируем: у обоих
longhorn-классов `allowVolumeExpansion: true`, и смена `spec.storage.size`
прокатывается оператором на PVC автоматически, а Longhorn расширяет том на ходу,
без пересоздания пода. Обратно том не сжимается, поэтому запас — не бесплатная
опция: Longhorn учитывает запрошенный размер при планировании.

Ориентиры: `shared` — пол + 132 МБ ≈ 171 МБ (том 1Gi), `zitadel` — пол + 20 МБ
≈ 59 МБ (256Mi), `immich` — пол + 287 МБ и растёт с библиотекой (1Gi),
`dawarich` — пол + 93 МБ (512Mi).

## Расширения

Расширения различаются по области действия, и это определяет границы кластеров:

- **Уровень базы** (`CREATE EXTENSION`): `dawarich` — PostGIS. Соседи его не
...
- **Уровень кластера** (`shared_preload_libraries`): `immich` — VectorChord.
...
устаревший, `standard` + бинари Barman Cloud. `immich` берёт `standard`, потому
...
не пересекает мажоры. Кластеры с расширениями (`immich`, `dawarich`) ждут
...
- **Second node.** При появлении второй ноды: `instances: 3` для `zitadel`

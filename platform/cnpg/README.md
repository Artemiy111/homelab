...
- **Synapse — исключение, которому нужен строго `C`.** `PostgresEngine.check_database`
  требует `datcollate = datctype = 'C'` ровно: даже `C.UTF-8` он считает ошибкой
  (`IncorrectDatabaseSetup`), поэтому у базы `synapse` в `shared` collate и ctype
  заданы как `C`, а не дефолт кластера. Шаблон при создании — `template0`:
  дефолтный `template1` навязывает ctype кластера (`C.utf8`), а от него другую
  локаль PostgreSQL не даёт задать (SQLSTATE 22023). Поля `template`,
  `localeCollate`/`localeCType` у `Database` иммутабельны, но база переносится
  пустой, так что пересоздание CR ничего не теряет.
...
- **Метрики.** У оператора нет CRD prometheus-operator, поэтому PodMonitor не
  ...
  `playground`, `authentik`, `forgejo`, `sure`, `nextcloud` и `element`.

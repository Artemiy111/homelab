...
`https://id.example.net/`; порт контейнера на хосте не публикуется.
...
docker compose ps
```
Откройте `https://id.example.net/setup`, создайте администратора и сразу
...
```sh
docker compose ps
curl --resolve id.example.net:443:192.0.2.10 \
  -fsS https://id.example.net/.well-known/openid-configuration
docker logs --since=5m pocket-id 2>&1
```

OIDC discovery должен вернуть JSON с issuer
`https://id.example.net`. После первоначальной настройки добавьте
